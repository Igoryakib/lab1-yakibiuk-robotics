...
class FigureEightPath(Node):
    def __init__(self):
        ...
        # Параметри (швидкості та множник підібрані для повного кола)
        self.declare_parameter("linear_speed", 0.3)
        self.declare_parameter("angular_speed", 0.3)
        self.declare_parameter("time_multiplier", 3.0) 
        ...
        # Стан: 0 - очікування, 1 - перше коло (ліворуч), 2 - друге коло (праворуч)
        self.state = 1
    ...
    def timer_callback(self):
        ...
        if self.state == 1:
            # Перше коло: позитивна кутова швидкість (вліво)
            ...
            # Друге коло: негативна кутова швидкість (вправо)
            if elapsed <= self.circle_duration:
                msg.twist.linear.x = self.v
                msg.twist.angular.z = -self.w # Зміна знаку!
                self.pub.publish(msg)
            else:
                self.stop_robot()
